...
import { CARD_LIST_SIZE } from '../constants/ui.constant'
import { ImageDetailsList } from '../components/ImageDetailList'
import useSWR from 'swr'
import { UNSPLASH_API_URL } from '../constants/api.constant'
...
const InstagramIOSPageTransitionsHomeScreen = ({ animatedRef, active, pageX, pageY, headerHeight }) => {
  const _headerHeight = useHeaderHeight()
  const [pageIndex, setPageIndex] = useState(2)
  const [feedData, setFeedData] = useState([])

  const { data: unsplashData } = useSWR<ResponseProps>(`${UNSPLASH_API_URL}&page=${pageIndex}`, fetcher)
  ...
  return (
    <>
      ...
      <Portal hostName='DetailsScreen'>
        <ImageDetailsList list={feedData} active={active} headerHeight={headerHeight} />
      </Portal>
    </>
  )
}

export default InstagramIOSPageTransitionsHomeScreen
