...
import { SPRING_CONFIGURATION } from '../constants/animations.constant'
...
const getHipotenus = (offsetX, offsetY) => {
  ...
  const distanceHipo =
    offsetX.value === 0 && offsetY.value < 0
      ...
      : Math.round(Math.sqrt(offsetX.value * offsetX.value + _offsetYValue * _offsetYValue))
  return Math.sqrt(offsetX.value * offsetX.value + offsetY.value * offsetY.value)
}
